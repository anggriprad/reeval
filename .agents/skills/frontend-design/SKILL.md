...
Before creating new UI elements or writing raw HTML elements (`<select>`, `<input>`, `<button>`, custom modal overlays, custom badges, raw tables, etc.):

1. **Always inspect `@/components/ui/` first**: The list of reusable components in `@/components/ui/` is continuously growing. Always inspect the folder directly (`src/components/ui/`) before implementing UI logic to discover all currently available design system components (e.g. `Button`, `Input`, `Select`, `Modal`, `Card`, `Badge`, `CurrencyInput`, `SegmentedControl`, `ConfirmDialog`, `EmptyState`, etc.).
2. **Never re-invent or bypass existing UI components**: Always use available UI components from `@/components/ui/` to guarantee design consistency, keyboard accessibility, dark mode styling, and proper component props across all pages.
3. **Extend or compose components cleanly**: If an existing component is missing a minor variant or prop needed for a page, extend the component cleanly in `@/components/ui/` rather than creating inline one-off HTML elements.
4. **Always use Modal `actions` prop**: Whenever using `@/components/ui/Modal.tsx`, ALWAYS pass modal action buttons (such as submit, confirm, cancel, close, delete buttons) to the `actions` prop of `<Modal>` instead of placing custom action button containers inside `children`. This ensures action buttons stay fixed at the bottom footer with consistent padding, dark mode backdrop styling, and proper layout.
